...
class VAEPredictor:
    """
    Predictor class for training and testing model.

    Args:
        config (dict): A dictionary containing configuration parameters for the predictor.

    Attributes:
        config (dict): A dictionary containing model and data parameters.
        model (torch.nn.Module): The pytorch model.
        device (torch.device): The computing device (cpu or cuda) for training.

    Methods:
        train(expt_name, project_name, find_lr):
            Trains the machine learning model.

        test(expt_name, project_name):
            Tests the model with the given experiment name and project name.

        run_inference(dataset):
            runs the given model on a dataset using the specified device.

        load(expt_name, project_name):
            Loads a trained model given the experiment name and project name
