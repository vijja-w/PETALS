...
import numpy as np
from method_order.utils import model_init, data_init
...
class Predictor:
        ...
        for epoch in range(max_epoch):
            ...
            with torch.no_grad():
                for batch_idx, batch in enumerate(val_dataloader):
                    loss, n_correct = self.model.validation_step(batch, batch_idx)
                    valid_loss += loss.item()
                    total_correct += n_correct
                    total += batch[0].shape[0]

            wandb.log({"val_loss": valid_loss})
            wandb.log({"val_accuracy": total_correct/total})
    ...
    def run_inference(self, dataset):
        ...
        for i, s in enumerate(dataset.s_list):
            data[s] = []
            for t in dataset.s2t_list[s]:
                image = dataset.get_item(t, s).to(self.device)
                z = self.model(image).detach().cpu().numpy()
